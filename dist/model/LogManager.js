...
export class LogManager {
    ...
    async getLogs(data = {}) {
        try {
            const { lines = 100, level, search, since, includeRaw = false } = data;
    ...
    async setLogLevel(data) {
        try {
            ...
            if (!this.config.mcp)
                this.config.mcp = {};
            if (!this.config.mcp.debug)
                this.config.mcp.debug = {};
    ...
    async exportLogs(data = {}) {
        try {
            const { format = 'json', level, search, since } = data;
            let logs = [...this.logBuffer];
            // 应用过滤器
            if (since) {
                const sinceTime = new Date(since).getTime();
            ...
            if (format === 'json') {
                ...
                const rows = logs.map(log => `${log.timestamp},"${new Date(log.timestamp).toISOString()}","${log.level}","${log.message.replace(/"/g, '""')}"`).join('\n');
                exportData = headers + rows;
            }
            else if (format === 'text') {
                exportData = logs.map(log => `[${new Date(log.timestamp).toISOString()}] ${log.level.toUpperCase()}: ${log.message}`).join('\n');
            }
            else {
