...
│  ├─ DEVELOPMENT.md
│  └─ API.md
├─ model/src/            # 全部 TS 源码（不直接运行）
│  ├─ MCPServer.ts
│  ├─ MCPHandler.ts
│  ├─ mcp-server.ts
│  ├─ agent/Simulator.ts
│  └─ model/*.ts
└─ model/dist/           # 运行期加载的编译产物
   ├─ MCPServer.js
   ├─ MCPHandler.js
   ├─ mcp-server.js
   ├─ agent/Simulator.js
...
- Yunzai 加载 `index.js` → 从 `model/dist/` 加载 `MCPServer.js`（复用 Bot.express）→ 注册 `/MCP` 路由 + WebSocket → 暴露 `global.mcpServer`。
- 对外 API（HTTP/WS）进入 `MCPServer` → 校验/记录 → 转发到 `MCPHandler.handleAction(action, data, ctx)`。
- `MCPHandler`（来自 `model/dist/`）内部通过 `actionMap` 调用各 Manager 完成具体操作。
...
- 独立 `mcp-server` 进程由 `index.js` 启动，产物路径为 `model/dist/mcp-server.js`。
