...
- TS 源码编译到 `model/dist/`，运行期仅加载 `model/dist/` 产物，不再加载 `lib/` JS。
...
- 运行期入口仍为 `index.js` 与 `apps/*.js`，但其内部仅引用 `model/dist/` 产物（例如：`./model/dist/MCPServer.js`）。
...
- 必须先执行构建，运行期只会加载 `model/dist/` 产物，不再提供 JS 降级路径。
- Windows/WSL 均可；如使用 WSL，请保证时钟与权限一致、工作目录位于挂载盘性能可接受的位置。

运行期如何使用 TS 编译结果

- `MCPHandler` 强制依赖 `model/dist/agent/Simulator.js`，不再做缺省降级。
  - 注册以下 MCP 动作：
...
- `apps/` 目录保持 JS，装载逻辑不变；其它模块一律 TS 并从 `model/dist/` 加载。
- 迁移准则：新功能与重构先落 TS（src），通过 `npm run build` 产出后由 JS 入口引用。

故障排查

- “找不到 model/dist 模块”：未执行 `npm run build`，或构建失败。
...
- 请参考 `docs/MCP-CLIENT.md`，根据你的操作系统将 IDE 的 MCP 配置指向 `model/dist/mcp-server.js`，并设置所需环境变量。
