...
export class MCPHandler {
  ...
  setupActionMap() {
    this.actionMap = {
      ...
      'bot.restart': this.handleBotRestart.bind(this),
      ...
      'logs.get': this.handleGetLogs.bind(this),
      'logs.stream': this.handleStreamLogs.bind(this),
      'message.response': this.handleGetMessageResponse.bind(this),
      'time.get': this.handleGetTime.bind(this)
    };
  }
  ...
  setupLogCapture() {
    ...
    if (global.logger) {
      ...
      ['info', 'warn', 'error', 'debug', 'mark'].forEach(level => {
        if (typeof global.logger[level] === 'function') {
          originalLoggerMethods[level] = global.logger[level];
          global.logger[level] = (...args) => {
            this.captureLog(level, args);
            originalLoggerMethods[level].apply(global.logger, args);
          };
        }
      });
    }
  ...
  async handleAction(action, data, context) {
    ...
    switch (action) {
      ...
      // 日志相关
      case 'logs.get':
        return await this.handleGetLogs(data);
      case 'logs.stream':
      ...
      // 时间相关
      case 'time.get':
        return await this.handleGetTime(data);
  ...
  async handleGetLogs(data) {
    ...
    return {
      ...
      filters: { lines, level, since, search },
      timestamp: Date.now()
    };
  }
  ...
  async handleClearLogs() {
    ...
    this.captureLog('info', ['[MCP] 日志缓冲区已清理', { clearedCount }]);
