...
export class MockEnvironment {
    ...
    ensureFriend(user_id, nickname = 'Mock Friend') {
        ...
        if (!this.friends.has(uid))
            this.friends.set(uid, { user_id: uid, nickname });
        return this.friends.get(uid);
    }
    ensureGroup(group_id, name = 'Mock Group') {
        const gid = String(group_id);
        if (!this.groups.has(gid))
            this.groups.set(gid, { group_id: gid, name });
        if (!this.groupMembers.has(gid))
            this.groupMembers.set(gid, new Map());
            ...
            if (Buffer.isBuffer(item.file)) {
                ...
                if (fileStr.startsWith('base64://')) {
                    ...
                    if (fetched) {
                        ...
                        let remoteName = '';
                        try {
                            remoteName = path.basename(new URL(fileStr).pathname || '');
                        }
                        catch { }
                ...
                if (fetched) {
                    ...
                    let remoteName = '';
                    try {
                        remoteName = path.basename(new URL(item.url).pathname || '');
                    }
                    catch { }
                    await finalize(buffer, fileName || remoteName);
                }
    ...
    removeMember({ group_id, user_id }) {
        ...
        const gm = this.groupMembers.get(gid);
        if (!gm)
            return { removed: false, reason: 'group_not_found' };
    ...
    buildFriendApi(friend) {
        ...
        return {
            ...
            async sendMsg(msg) {
                ...
                try {
                    env._attachToRecentBuffer('private', friend.user_id, msg);
                }
                catch { }
    ...
    buildGroupApi(group) {
        ...
        return {
            ...
            async sendMsg(msg) {
                ...
                try {
                    env._attachToRecentBuffer('group', group.group_id, msg);
                }
                catch { }
    ...
    findMessage(message_id) {
        ...
        return (this.outbox.find(m => m.message_id === mid) ||
    ...
    history({ type, target, limit = 50 }) {
        ...
        if (tg)
            list = list.filter(x => String(x.target) === tg);
    ...
    _attachToRecentBuffer(type, target, msg) {
        ...
        if (!buf || typeof buf.forEach !== 'function')
            return;
        let bestKey = null;
        let bestTs = 0;
        buf.forEach((val, key) => {
        ...
        if (bestKey && buf.has(bestKey)) {
            const entry = buf.get(bestKey);
            const last = entry.responses?.[entry.responses.length - 1];
            const lastSignature = last ? this.stringifyForLog(last.content) : null;
            const incomingSignature = this.stringifyForLog(msg);
            if (!lastSignature || lastSignature !== incomingSignature) {
                entry.responses.push({ type: 'reply', content: msg, timestamp: Date.now() });
            }
        }
    }
        ...
        const fmtItem = async (item) => {
            ...
            if (typeof item === 'object' && item) {
                if (item.type === 'image') {
                    ...
                    if (!local && item.mcpLocalPath)
                        local = item.mcpLocalPath;
                    const lines = [
                        `发送图片: ${label}`
                    ];
                    if (local)
    ...
    async incomingMessage({ message, user_id, group_id, nickname, role = 'member', waitMs = 1200, traceId }) {
        ...
        if (isGroup)
            event.group = groupApi;
        else
            event.friend = friendApi;
        // reply: push into messageResponseBuffer via handler
        event.reply = (msg) => (isGroup ? groupApi.sendMsg(msg) : friendApi.sendMsg(msg));
        // Pre-register for response tracking
        try {
            this.handler?.recordMessageForResponse?.(event);
        }
        catch { }
        // Log and emit
        try {
